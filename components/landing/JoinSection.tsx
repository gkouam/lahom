...
export default function JoinSection() {
  ...
  const handleJoin = async () => {
    if (!email.trim()) {
      setMsg({ text: t('form.emailEmpty'), type: 'error' })
      return
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      setMsg({ text: t('form.emailInvalid'), type: 'error' })
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        setMsg({ text: t('form.success'), type: 'success' })
        setEmail('')
  ...
  return (
    <section className="section join-section" id="join">
      <div className="join-pattern-bg"></div>
      <div className="join-inner fade-in">
        <span className="sec-label gold">{t('join.label')}</span>
        <h2 className="sec-title light" style={{ textAlign: 'center' }} dangerouslySetInnerHTML={{ __html: t('join.title') }} />
        <p>{t('join.desc')}</p>
        ...
        <a
          href="https://wa.me/14695550000?text=Hello%2C%20I%20want%20to%20join%20the%20Baham%20community"
